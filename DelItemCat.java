  ...
  extends HttpServlet
{
    ...
    throws ServletException, IOException
  {
    ...
    pw.println("<form method='POST' action='DelCategory'>");
    pw.println("<center><h2>ItemCategory List</h2><br>");
    try
    {
      Statement st = con.createStatement();
      for (ResultSet rs = st.executeQuery("select cat_name from category_master"); rs.next(); pw.println("<table><tr><td>" + category + "</td><td><input type=checkbox name=chk value='" + category + "'></td></tr></table><br>")) {
        category = rs.getString(1);
      }
      pw.println("<center><input type=submit value='Delete' > </form>");
    }
    catch (Exception e)
    {
      e.printStackTrace();
